...
async def get_setting_by_slug(setting_slug: str, db: Session = Depends(get_db)):
...
async def update_setting(setting_id: int, setting: SettingUpdate, db: Session = Depends(get_db)):
